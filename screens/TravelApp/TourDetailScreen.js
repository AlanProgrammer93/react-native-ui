...
const TourDetailScreen = ({ tour, setTourDetail }) => {
	const [showImage, setShowImage] = useState(tour?.image)
	return (
		<>
			<ScrollView>
				<ImageBackground
								...
								<View>
									{tour?.images.map((gallery, index) => (
										<TouchableOpacity
											...
											key={index}
											onPress={() => setShowImage(gallery.image)}
										>
											<Image
			...
			<View
				style={{ position: "absolute", bottom: SPACING, width: "100%" }}
			>
				<TouchableOpacity
					style={{
